...
static pthread_mutex_t entry_lock;
static pthread_mutex_t inode_lock;
static pthread_mutex_t free_blocks_counter;
...
int free_blocks_cunt(){
    int c = 0;

    pthread_mutex_lock(&free_blocks_counter);
    for(int i=0; i<DATA_BLOCKS; i++){ 
        if (free_blocks[i] == FREE) {
            c++;
        }
    }
    pthread_mutex_unlock(&free_blocks_counter);
    return c;
}
    ...
    for (int inumber = 0; inumber < INODE_TABLE_SIZE; inumber++) {
        ...
        if (freeinode_ts[inumber] == FREE) {
            ...
            if (n_type == T_DIRECTORY) {
                ...
                pthread_rwlock_init(&inode_table[inumber].inode, NULL);
                inode_table[inumber].i_size = 0;
                /*maybe a for loop for seting allvalues*/
